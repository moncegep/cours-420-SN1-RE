...
def generer_recommandations(matrice, sous_categories, var_pandemie):
    """
    ...
    - Quelle catégorie numérique a le plus augmenté entre 2017 et 2023?
...
def graphique_potentiel(potentiel, chemin_sortie):
    """
    ⭐ OPTIONNEL — À faire si les deux premiers graphiques sont terminés.
...
def main():
    ...
    # TODO - Génère et sauvegarde les 2 graphiques obligatoires (+ le 3e si optionnel fait)
    ...
```
...
2. **Rapport écrit** (2-3 pages) + **dossier `graphiques/`** avec les 2 graphiques obligatoires (+ le 3e si réalisé)
...
| Calculs NumPy corrects — `axis`, `argmax`, `argsort`, `std`, gestion des `nan` | 20 |
