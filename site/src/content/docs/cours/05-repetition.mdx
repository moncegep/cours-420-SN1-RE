...
Ça fonctionne, mais c'est pénible. Et si on voulait aller jusqu'à 100 ? Ou jusqu'à un nombre choisi par l'utilisateur ? Il faudrait recopier des lignes sans fin — ou pire, ne pas pouvoir le faire du tout.
...
- Un jeu vidéo qui **répète** la boucle de jeu tant que le joueur n'a pas perdu
...
Le flux ne descend plus simplement de haut en bas : il **remonte** pour ré-exécuter les mêmes instructions. La boucle continue tant que la condition est vraie, puis le programme reprend son cours normal.
...
La version avec boucle est plus courte, et surtout **elle s'adapte** : change `10` par `1000` et ça fonctionne immédiatement, sans ajouter 990 lignes.
...
Ici, c'est **impossible** de savoir à l'avance combien de fois la boucle va s'exécuter — ça dépend de l'utilisateur. C'est exactement le type de situation où `while` est indispensable.

---

### Validation de saisie

Un des usages les plus courants du `while` : **redemander une valeur** tant qu'elle n'est pas valide.
...
La saisie est faite **deux fois** dans le code : une fois avant la boucle et une fois à l'intérieur. C'est un peu redondant, mais c'est le patron classique de validation avec `while`. Tu verras dans la section suivante une façon de l'éviter.
</Aside>
...
Les trois exemples ci-dessus sont du **pseudo-code** — ils illustrent le patron général mais ne fonctionneraient pas tels quels en Python. Le patron `while True` + `break` est cependant bien réel et tu l'utiliseras souvent.
</Aside>
